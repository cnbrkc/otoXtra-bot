...
- 🔍 **Otomotiv konu doğrulama** — AI alakasız haberleri (Bim/A101 kataloğu vb.) daha puanlama aşamasına gelmeden eler
- Akıllı tekrar/benzerlik kontrolü (URL + başlık + konu parmak izi)
- Logo watermark ekleme
- Anti-bot stratejisi (rastgele gecikme, atlama)
- Telegram bildirimi (başarılı/başarısız paylaşım + haftalık rapor)
- Telegram manuel paylaşım kuyruğu (görsel + açıklama gönder, otomatik paylaşsın)
- Test modu desteği
...
| `TELEGRAM_BOT_TOKEN` | Telegram bot token (bildirim + manuel kuyruk için) |
...
| Alakasız haber paylaşıyor (Bim, A101 vb.) | `keywords.json` include listesini kontrol et, `prompts.json` otomotiv dışı kuralı aktif |
...
├── platforms/        → Facebook + Telegram API katmanı
