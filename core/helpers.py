...
def generate_topic_fingerprint(title: str) -> str:
    ...
    keywords = sorted(w for w in words if len(w) >= 3 and _normalize_token(w) not in _NORMALIZED_STOP_WORDS)
...
def _extract_keywords_from_title(title: str, min_len: int = 4) -> Set[str]:
...
def is_duplicate_article(article1: Dict[str, Any], article2: Dict[str, Any]) -> bool:
    try:
        settings = load_config("settings")
    ...
    if url1 and url2 and url1 == url2: return True
    if is_similar_title(title1, title2, threshold=title_threshold): return True
...
def get_previous_week_key(reference_dt: Optional[datetime] = None) -> str:
    return _get_week_key((reference_dt or get_turkey_now()) - timedelta(days=7))

def _ensure_stats_schema(data: Dict[str, Any]) -> None:
    stats = data.setdefault("stats", {})
    if not isinstance(stats, dict):
        data["stats"] = {}
        stats = data["stats"]
    if not isinstance(stats.get("daily_actions"), dict): stats["daily_actions"] = {}
    if not isinstance(stats.get("weekly"), dict): stats["weekly"] = {}

def _ensure_weekly_bucket(data: Dict[str, Any], week_key: str) -> Dict[str, Any]:
...
def increment_action_trigger(posted_data: Dict[str, Any]) -> int:
...
def get_today_action_count(posted_data: Dict[str, Any]) -> int:
    _ensure_stats_schema(posted_data)
    return int(posted_data["stats"]["daily_actions"].get(get_today_str(), 0))

def increment_weekly_share(posted_data: Dict[str, Any]) -> None:
    week_bucket = _ensure_weekly_bucket(posted_data, _get_week_key())
    week_bucket["shares"] = int(week_bucket.get("shares", 0)) + 1

def record_weekly_error(posted_data: Dict[str, Any], error_code: str, error_name: str = "") -> None:
    week_bucket = _ensure_weekly_bucket(posted_data, _get_week_key())
...
def record_weekly_skip(posted_data: Dict[str, Any], skip_reason: str = "") -> None:
    week_bucket = _ensure_weekly_bucket(posted_data, _get_week_key())
...
def get_weekly_stats(posted_data: Dict[str, Any], week_key: str) -> Dict[str, Any]:
    bucket = _ensure_weekly_bucket(posted_data, week_key)
    return {
        "actions": int(bucket.get("actions", 0)), "shares": int(bucket.get("shares", 0)),
        "error_total": int(bucket.get("error_total", 0)), "errors": dict(bucket.get("errors", {})),
        "skip_total": int(bucket.get("skip_total", 0)), "skips": dict(bucket.get("skips", {})),
        "report_sent": bool(bucket.get("report_sent", False)), "report_sent_at": bucket.get("report_sent_at"),
    }

def is_weekly_report_sent(posted_data: Dict[str, Any], week_key: str) -> bool:
    return bool(_ensure_weekly_bucket(posted_data, week_key).get("report_sent", False))

def mark_weekly_report_sent(posted_data: Dict[str, Any], week_key: str) -> None:
    bucket = _ensure_weekly_bucket(posted_data, week_key)
...
def get_token_remaining_days() -> Optional[int]:
...
def get_posted_news() -> Dict[str, Any]:
    ...
    if not isinstance(data.get("daily_counts"), dict): data["daily_counts"] = {}
...
def save_posted_news(data: Dict[str, Any]) -> bool:
    ...
    cleaned_daily = {date: count for date, count in daily_counts.items() if date >= cutoff_date_str}
    ...
    stats["daily_actions"] = {date: count for date, count in daily_actions.items() if date >= cutoff_date_str}
...
def _cooldown_key(url: str, title: str) -> str:
    ...
    if fingerprint: return f"fp:{fingerprint}"
    normalized_url = (url or "").strip().lower()
    return f"url:{normalized_url}" if normalized_url else ""

def record_shared_variant_cooldown(posted_data: Dict[str, Any], posted_article: Dict[str, Any], variant_article: Dict[str, Any], reason: str = "shared_variant") -> None:
    ...
    cooldowns[key] = {
        "title": variant_article.get("title", ""), "url": variant_article.get("link", ""),
        "score": variant_article.get("score", 0), "reason": (reason or "shared_variant").strip()[:160],
...
def is_shared_variant_in_cooldown(url: str, title: str, posted_data: Dict[str, Any], cooldown_hours: int) -> bool:
...
def is_topic_already_posted(fingerprint: str, posted_data: Dict[str, Any], similarity_threshold: float = 0.45) -> bool:
